...
const ctrlCarts = {
  allCart: async (req, res) => {
    ...
    try {
      const cartAllList = await Cart.findAll({ where: { userId } });
  ...
  updateCart: async (req, res) => {
    ...
    // 예외 상황2 : 갯수 1개에서 0개로
    if (quantity == 0) {
      await Cart.destroy({ where: { [Op.or]: [{ userId }, { postId }] } });
      return res.status(200).send({
        result: "success",
    ...
    const bfChgCart = await Cart.findOne({
      where: { [Op.or]: [{ userId }, { postId }] },
    });
    ...
    try {
      await Cart.update({ userId, postId }, { where: { quantity } });
      const atChgCart = await Cart.findOne({
        where: { [Op.or]: [{ userId }, { postId }] },
      });
